...
class UserApi extends Api
{
    ...
    public function show()
    {
        ...
        $user_info['medal_count'] = count($list);
        foreach ($list as $v) {
        ...
        //用户部门儿
        $user_info['department_list'] = '';
        $userInfo['department_id'] && $user_info['department_list'] = getDepartmentById($userInfo['department_id']);
    ...
    public function user_friend()
    {
        ...
        if (!empty($friend)) {
            ...
            foreach ($friend as $k => $v) {
                ...
                $friends = array(
                    'follow_id' => $v['follow_id'],
                    'uid' => $friend_info ['uid'],
                    'uname' => $friend_info ['uname'],
                    'intro' => $friend_info ['intro'] ? formatEmoji(false, $friend_info ['intro']) : '',
                    'avatar' => $friend_info ['avatar']['avatar_big'],
                    'follow_status' => $follow_status [$v ['fid']],
                );
                $key = getShortPinyin($friends['uname']);
                if (!isset($friend_arr[$key])) {
                    $friend_arr[$key] = array();
                }
                if (!in_array($key, $keys)) {
                    array_push($keys, $key);
                }
                array_push($friend_arr[$key], $friends);
            }
            asort($keys);
            $retArr = array();
            foreach ($keys as $val) {
                $retArr[$val] = $friend_arr[$val];
            }
            unset($friend_arr);

            return array('status' => 1, 'msg' => '获取成功', 'data' => $retArr);
        } else {
            return array('status' => 0, 'msg' => '还没有互相关注的好友', 'data' => '');
        }
    }
