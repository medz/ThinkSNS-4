...
';
    ...
    {
        $where = '`is_del` = 0 AND `is_audit` = 1 AND `app` = \'Information\' AND `table` = \'%s\' AND `row_id` = %d';
    ...
    public function addComment()
    {
        ...
        if (!$filterContentStatus['status']) {
            $return ['status'] = 0;
            $return ['msg'] = $filterContentStatus['data'];
            return $return;
        }
...
.wrap .content { width: 100%; max-width: 100%; height: auto; overflow-x: hidden; color: #3e3e3e;line-height: 1.4;}
.content img{max-width:100%!important;}
